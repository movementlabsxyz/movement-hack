...
module ds_std::std_set {
    ...
    struct StdSet<K: copy + drop> has copy, drop, store {
        contents: vector<K>,
    }

    /// Create an empty `StdSet`
    public fun empty<K: copy + drop>(): StdSet<K> {
        StdSet { contents: vector::empty() }
    }

    /// Create a singleton `StdSet` that only contains one element.
    public fun singleton<K: copy + drop>(key: K): StdSet<K> {
        StdSet { contents: vector::singleton(key) }
    }

    /// Insert a `key` into self.
    /// Aborts if `key` is already present in `self`.
    public fun insert<K: copy + drop>(self: &mut StdSet<K>, key: K) {
    ...
    public fun remove<K: copy + drop>(self: &mut StdSet<K>, key: &K) {
        let idx = get_idx(self, key);
        vector::remove(&mut self.contents, idx);
    }

    /// Return true if `self` contains an entry for `key`, false otherwise
    public fun contains<K: copy + drop>(self: &StdSet<K>, key: &K): bool {
        option::is_some(&get_idx_opt(self, key))
    }

    /// Return the number of entries in `self`
    public fun size<K: copy + drop>(self: &StdSet<K>): u64 {
        vector::length(&self.contents)
    }

    /// Return true if `self` has 0 elements, false otherwise
    public fun is_empty<K: copy + drop>(self: &StdSet<K>): bool {
        size(self) == 0
    }

    /// Unpack `self` into vectors of keys.
    /// The output keys are stored in insertion order, *not* sorted.
    public fun into_keys<K: copy + drop>(self: StdSet<K>): vector<K> {
        let StdSet { contents } = self;
        contents
    }

    /// Borrow the `contents` of the `StdSet` to access content by index
    /// without unpacking. The contents are stored in insertion order,
    /// *not* sorted.
    public fun keys<K: copy + drop>(self: &StdSet<K>): &vector<K> {
        &self.contents
    }
    ...
    fun get_idx_opt<K: copy + drop>(self: &StdSet<K>, key: &K): Option<u64> {
        let i = 0;
        let n = size(self);
        while (i < n) {
    ...
    fun get_idx<K: copy + drop>(self: &StdSet<K>, key: &K): u64 {
